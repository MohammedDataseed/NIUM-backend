...
export class PdfService {
  ...
  async uploadDocumentByOrderId(
    ...
    if (buffer.length > this.MAX_SIZE_BYTES)
      throw new BadRequestException("File size must be ≤ 1MB");
  ...
  async mergeFilesByFolder(
    ...
    try {
      ...
      const files = response.Contents || [];
      console.log(
        `Found ${files.length} files in S3: ${JSON.stringify(
          files.map((f) => f.Key)
        )}`
      );
      ...
      for (const file of files.filter(
        ...
        console.log(
          `Downloaded file ${file.Key}, size: ${fileData.length} bytes`
        );
        ...
        if (ext === "pdf") {
          ...
          fileData = await sharp(fileData)
            ...
            .toBuffer();
          console.log(
            `Compressed image ${file.Key}, new size: ${fileData.length} bytes`
          );
          ...
          copiedPages.forEach((page) => mergedPdf.addPage(page));
          console.log(
            `Added ${copiedPages.length} pages from converted ${file.Key}`
          );
        }
      }
      ...
      if (newFileBuffer && newFileMimeType) {
        console.log(
          `Processing new file, MIME type: ${newFileMimeType}, size: ${newFileBuffer.length} bytes`
        );
        ...
        if (newFileMimeType === "application/pdf") {
          ...
          processedBuffer = await sharp(newFileBuffer)
            ...
            .toBuffer();
          console.log(
            `Compressed new image, new size: ${processedBuffer.length} bytes`
          );
          const pdfBuffer = await this.convertImageToPdf(
            processedBuffer,
            "jpg"
          );
          const imgPdf = await PDFDocument.load(pdfBuffer);
          ...
          copiedPages.forEach((page) => mergedPdf.addPage(page));
          console.log(
            `Added ${copiedPages.length} pages from converted new image`
          );
        }
      }
  ...
  private async convertImageToPdf(
    ...
    try {
      const base64Image = `data:image/${ext};base64,${imageBuffer.toString(
        "base64"
      )}`;
