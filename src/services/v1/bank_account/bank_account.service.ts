...
export class BankAccountService {
  ...
  async findAll(
    ...
    params: WhereOptions<bank_account>,
  ): Promise<bank_account[]> {
    const childSpan = span
      .tracer()
      .startSpan('find-all-bank-accounts', { childOf: span });
  ...
  async findOne(span: opentracing.Span, id: string): Promise<bank_account> {
    const childSpan = span
      .tracer()
      .startSpan('find-bank-account', { childOf: span });
  ...
  async createBankAccount(
    ...
    const childSpan = span
      .tracer()
      .startSpan('create-bank-account', { childOf: span });
  ...
  async updateBankAccount(
    ...
    const childSpan = span
      .tracer()
      .startSpan('update-bank-account', { childOf: span });
  ...
  async deleteBankAccount(span: opentracing.Span, id: string): Promise<void> {
    const childSpan = span
      .tracer()
      .startSpan('delete-bank-account', { childOf: span });
