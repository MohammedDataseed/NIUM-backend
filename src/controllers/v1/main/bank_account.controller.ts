import {
  ...
  export class BankAccountController {
    ...
    @ApiResponse({ status: 200, description: "Bank Account details", type: bank_account })
    ...
    @ApiParam({ name: "id", description: "Bank Account ID", type: "string" })
    @ApiResponse({ status: 200, description: "Bank account successfully deleted" })
    @ApiResponse({ status: 404, description: "Bank account not found" })
    async deleteBankAccount(@Param("id") id: string): Promise<{ message: string }> {
      const tracer = opentracing.globalTracer();
